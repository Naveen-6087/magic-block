...
describe("anchor-counter", () => {
  ...
  // Using Asia endpoint for better performance from India
  const providerEphemeralRollup = new anchor.AnchorProvider(
    new anchor.web3.Connection(
      process.env.PROVIDER_ENDPOINT || "https://devnet-asia.magicblock.app/",
      {
        wsEndpoint: process.env.WS_ENDPOINT || "wss://devnet-asia.magicblock.app/",
      }
    ),
    anchor.Wallet.local()
  );
  ...
  const program = anchor.workspace.Counter as Program<Counter>; // Changed from AnchorCounter
